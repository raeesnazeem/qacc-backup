 ...
 */
export function formatBasecampCampfireMention(sgid: string, name: string): string {
...
export async function createBasecampCampfireLine(params: {
  ...
  try {
    ...
    if (!chatTool) {
      throw new Error(`No Campfire (chat) tool found in project ${projectId}. Make sure the Campfire tool is enabled on the project!`);
    }

    const chatId = chatTool.id;
    const targetChat = chatTool;
    console.log(`[BasecampClient] Found Chat Tool in Dock:`, JSON.stringify(chatTool, null, 2));
