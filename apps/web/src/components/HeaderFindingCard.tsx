...
export const HeaderFindingCard: React.FC<FindingCardProps> = ({
  ...
  const [isSocialVerified, setIsSocialVerified] = React.useState(false)
  const [isMobileVerified, setIsMobileVerified] = React.useState(false)
  const [isEmailVerified, setIsEmailVerified] = React.useState(false)
  const [isStickyVerified, setIsStickyVerified] = React.useState(false)
  ...
  return (
    <div
      className={`group p-6 bg-slate-200/10 dark:bg-[#1D2A31] rounded-md border transition-all duration-300 relative overflow-hidden flex flex-col gap-6 ${isConfirmed || isAssigned || hasTask ? "border-emerald-500 ring-1 ring-emerald-500/20" : isFalsePositive ? "opacity-60 border-slate-200 dark:border-slate-800" : "border-slate-200 dark:border-slate-800 hover:border-accent/40"}`}
    >
      <div
        ...
        {screenshotUrls.length > 0 && (
          <div className="space-y-2 pt-2">
            ...
            <div className="flex items-start justify-between w-full">
              ...
              <div className="w-[35%] flex flex-col gap-2 pl-2 border-l border-slate-100 dark:border-slate-700/50 ml-5">
                <label className="flex items-center gap-2 group/cb">
                  <input
                    type="checkbox"
                    disabled={isPushed}
                    checked={isSocialVerified}
                    onChange={(e) => setIsSocialVerified(e.target.checked)}
                ...
                <label className="flex items-center gap-2 group/cb">
                  ...
                  />
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest group-hover/cb:text-slate-900 transition-colors cursor-pointer truncate">
                    {isEmailVerified ? "Email ID Verified" : "Verify Email ID"}
                  </span>
                </label>
                <label className="flex items-center gap-2 group/cb">
                  <input
                    type="checkbox"
                    disabled={isPushed}
                    checked={isStickyVerified}
                    onChange={(e) => setIsStickyVerified(e.target.checked)}
                    className="w-3 h-3 text-accent border-slate-300 dark:border-slate-600 dark:bg-[#131d22] rounded focus:ring-accent accent-accent cursor-pointer disabled:cursor-not-allowed transition-all"
                  />
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest group-hover/cb:text-slate-900 transition-colors cursor-pointer truncate">
                    {isStickyVerified
                      ? "Sticky Header Verified"
        ...
        <div className="pt-2 flex items-center justify-start gap-3">
              ...
              <button
                onClick={() => {
                  onCreateTask?.({
                    ...finding,
                    title: localTitle,
                    description: `Task Linked. Verified? Soc:${isSocialVerified} Mob:${isMobileVerified} Ema:${isEmailVerified} Sti:${isStickyVerified}`,
          ...
          {(hasTask || isAssigned) &&
            (() => {
              ...
              if (activeTaskIds.length === 0 || activeTaskIds[0] === finding.id)
                return null
