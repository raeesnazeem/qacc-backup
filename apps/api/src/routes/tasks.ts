...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (role === "developer" && !title.startsWith("[Feedback]")) {
      return res
        .status(403)
        .json({ error: "Developers can only create feedback tasks." })
    }
      ...
      if (assigned_to) {
        try {
          ...
          if (userData && projectData) {
            await emailNotifier.emailTaskAssigned(
              userData,
              task,
              projectData.name,
            )
          ...
          logger.error(
            err,
            `Failed to send assignment email for new task ${task.id}`,
          )
        }
      }
      ...
      try {
        ...
        if (performerRes.data && projectRes.data) {
          await activityService.logActivity(
            {
              id: performerRes.data.id,
              name: performerRes.data.full_name || "QA Engineer",
            },
            {
              type: "TASK_CREATED",
              details: {
...
router.get("/", clerkAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // RBAC: Sub-Admin/PM only see tasks in their project memberships (QA sees all org tasks)
    else if (
      role !== "super_admin" &&
      role !== "admin" &&
      role !== "qa_engineer"
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    const { id } = req.params
    const { status, assigned_to, description, gallery_images, severity } =
      req.body
    try {
      ...
      if (status) {
        try {
          ...
          if (
            ["resolved", "in-progress", "closed", "to-do"].includes(status) &&
            currentTask
          ) {
          ...
          logger.error(
            logError,
            "[ActivityService] Failed to log task status update",
          )
        }
      }
      ...
      if (req.body.hasOwnProperty("assigned_to")) {
        try {
          ...
          logger.error(
            logError,
            "[ActivityService] Failed to log assignment change",
          )
        }
      }
      ...
      if (status && currentTask) {
        const { data: siblings } = await supabase
          ...
          .or(
            `finding_id.eq.${currentTask.finding_id}${currentTask.finding_id ? "" : ",title.eq." + currentTask.title}`,
          )
        ...
        if (siblingIds.length > 0) {
          await supabase.from("tasks").update({ status }).in("id", siblingIds)
        }
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!inputId)
      return res
        .status(400)
        .json({ error: "user_id or assigned_to is required" })
    ...
    try {
      ...
      try {
        ...
        logger.error(
          logError,
          "[ActivityService] Failed to log task assignment",
        )
      }
    ...
    try {
      ...
      try {
        ...
        const [performerRes, taskRes] = await Promise.all([
          ...
          supabase
            .from("tasks")
            .select("title, created_by, assigned_to, project_id")
            .eq("id", id)
            .single(),
        ])
      ...
      try {
        ...
        if (taskRes.data) {
          await activityService.logActivity(
            {
              id: performerRes.data?.id || "",
              name: performerRes.data?.full_name || "Developer",
            },
            {
              type: "REBUTTAL_ADDED",
              details: {
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        logger.error(
          logError,
          "[ActivityService] Failed to log bulk task deletion",
        )
      }
