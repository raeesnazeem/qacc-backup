...
const CHECK_FACTOR_ICONS: Record<string, React.ReactNode> = {
  ...
  footer_logo: <Eye size={14} className="text-accent" />,
}

export const LogoCheckFindingCard: React.FC<FindingCardProps> = ({
  finding,
  pageScreenshots,
  onConfirm,
  onFalsePositive,
  ...
  const screenshotUrls = finding.screenshot_url
    ? finding.screenshot_url.split(",").map((url) => url.trim()).filter(Boolean)
    : []

  const allVerified = isDesktopVerified && isTabletVerified && isMobileVerified
            ...
            <>
              ...
              <button
                onClick={() => {
                  ...
                  const allAssigneesList = [...currentAssignees, ...assignedUsers]
                  const assigneeNames = Array.from(
                    new Set(
                      allAssigneesList
                        .map((u: any) =>
                  ...
                  const taskDescription = `Issue No: ${finding.id}
Assignees Tagged: ${assigneeNames || "None"}
Screenshots Verified:
Desktop: ${isDesktopVerified ? "Yes" : "No"}
Tablet: ${isTabletVerified ? "Yes" : "No"}
Mobile: ${isMobileVerified ? "Yes" : "No"}
`
